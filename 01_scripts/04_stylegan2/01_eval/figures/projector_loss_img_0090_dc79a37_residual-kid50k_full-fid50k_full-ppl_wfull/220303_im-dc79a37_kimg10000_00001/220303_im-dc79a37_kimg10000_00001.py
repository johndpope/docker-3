...
for name, item in latent_data.items():
    ...
    plt.plot(snapshot_kimg, normalize_01(metrics_dict["ppl_wfull"][:, np.newaxis]))
    img_number = int(name.split("_")[1])
    legend_name = [f"Projector Loss: Image {img_number}", "Kernel Inception Distance", "Frechet Inception Distance", "Perceptual path length in W"]
    plt.legend(legend_name)
    plt.xlabel("Number of k-images")
    plt.ylabel(r"Normalized metrics")
    fig_folder = "-".join([f"projector_loss_{name}", "kid50k_full", "fid50k_full", "ppl_wfull"]) 
...
plt.plot(snapshot_kimg, normalize_01(metrics_dict["fid50k_full"][:, np.newaxis]))
plt.plot(snapshot_kimg, normalize_01(metrics_dict["ppl_wfull"][:, np.newaxis]))
legend_name = ["Projector Mean-Loss", "Kernel Inception Distance", "Frechet Inception Distance", "Perceptual path length in W"]
plt.legend(legend_name)
plt.xlabel("Number of k-images")
plt.ylabel(r"Normalized metrics")

fig_folder = "-".join(["projector_loss_mean", "kid50k_full", "fid50k_full", "ppl_wfull"]) 
