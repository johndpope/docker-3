...
# Set to None if unused, dtype: np.array
rotation_deg_xyz = None

for rotation_deg_z in range(5,46,5):

    rotation_deg_xyz = [0,0, rotation_deg_z]

    g_dir = r"G:\ukr_data\Einzelzaehne_sorted\grid"
    p_dir = r"P:\MB\Labore\Robotics\019_ChewRob\99_Homes\bra45451\depo\docker\data\einzelzahn"
    ...
    if rotation_deg_xyz is not None:  
        ...
        g_dir = os.path.join(g_dir, "rotated", rot_folder)
        p_dir = os.path.join(p_dir, "rotated", rot_folder)
    ...
    for grid_size in grid_sizes:
        ...
        save_pathname = os.path.join(g_dir, param_hash, grid_folder)
        save_path_pcd = os.path.join(save_pathname, "pcd_grid")

        img_dir = os.path.join(p_dir, "images", param_hash, "grayscale",
                            grid_folder, "img")
